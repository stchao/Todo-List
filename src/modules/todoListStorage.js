...
export const TodoListStorage = (() => {
	...
	const _isLocalStorageAvailable = isStorageAvailable('localStorage');
	let _local = {};
	...
	const _set = () => {
		if (_isLocalStorageAvailable) {
			localStorage.setItem(_storageKey, JSON.stringify(_local));
		}
	};

	const get = () => _local;

	const getActiveProject = (getUuid = false) =>
		getUuid ? _activeProjectUuid : _local[_activeProjectUuid];
	...
	(function _loadInitialDataFromSession() {
		if (_isLocalStorageAvailable) {
			const tempStorageString = localStorage.getItem(_storageKey) ?? '{}';
